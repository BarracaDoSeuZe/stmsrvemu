...
def remove_from_dir(encrypted_buffer):
    ...
    if confirmation != b"\x00":
        log.warning(
            "Content Server failed to remove server to Content Server Directory Server "
        )
...
def _has_files(appid, version):
    paths_m = [
        ...
        os.path.join(
            config.get("v4manifestdir", "files/v4manifests/"),
            f"{appid}_{version}.manifest",
        ),
    ...
    paths_s = [
        ...
        os.path.join(
            config.get("v4storagedir", "files/v4storages/"), f"{appid}_{version}.dat"
        ),
        os.path.join("files/steam2_sdk_depots", f"{appid}_{version}.dat"),
    ]
    return any(os.path.isfile(p) for p in paths_m) and any(
        os.path.isfile(p) for p in paths_s
    )
        ...
        for file in files:
            ...
            try:
                if file.endswith(".bin"):
                    ...
                    record = (
                        contentdescriptionrecord.ContentDescriptionRecord.from_xml_file(
                            full
                        )
                    )
                ...
                continue
            apps = [
                int.from_bytes(k, "little") for k in record.ApplicationsRecord.keys()
            ]
            subs = [
                int.from_bytes(k, "little") for k in record.SubscriptionsRecord.keys()
            ]
            skip = False
            for app in apps:
                if app in used_apps:
                ...
                if app_rec:
                    for vr in app_rec.VersionsRecord.values():
                        ...
                        if not _has_files(app, ver):
                            log.error(
                                f"{file}: missing files for {app}_{ver}; skipping"
                            )
                            skip = True
                            break
                if skip:
...
def build_serverinfo(
    contentserver_info, applist, ispkg=False, used_apps=None, used_subs=None
):
...
def split_into_chunks(byte_string, chunk_size=512):
    ...
    chunks = [
        byte_string[i : i + chunk_size] for i in range(0, len(byte_string), chunk_size)
    ]
    return chunks
...
def handshake(client_socket, server_address, shared_secret, CLIENT_IDENTIFIER):
    ...
    client_socket.send(
        HEADER + b"\x01" + salt + message
    )  # Command byte \x01 for handshake
    command_byte = b"\x01"
...
def heartbeat_thread(server_instance):
    shared_secret = server_instance.config[
        "peer_password"
    ]  # Predefined shared secret for simplicity
    ...
    while True:
        ...
        log.error(
            "Failed to receive server response after 3 heartbeats. Attempting to reconnect..."
        )
        ...
        while True:
            ...
            try:
                ...
                client_socket.connect(server_address)
                key = handshake(
                    client_socket, server_address, shared_secret, CLIENT_IDENTIFIER
                )
                if not key:
                ...
                if not globalvars.aio_server:
                    ...
                    server_instance.used_app_ids = set(used.get("apps", []))
                    server_instance.used_sub_ids = set(used.get("subs", []))
                if not send_client_info(
                    client_socket, server_address, key, server_instance
                ):
